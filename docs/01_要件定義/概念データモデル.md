...
erDiagram
    ...
    Student ||--o{ HearingToken : "ヒアリングURL"
...
| chat_username | string | - | 連絡・識別用の任意項目（手入力） |

状態遷移の業務ルール・状態図は [ユースケース.md](ユースケース.md)（業務フロー概要の stateDiagram）を参照。
...
### 3.5 ヒアリングトークン（HearingToken）

| 属性 | 型 | 必須 | 説明 |
|------|-----|:----:|------|
| id | int | ✅ | 主キー |
| student_id | int | ✅ | 受講生（FK） |
| token | string | ✅ | URL 用トークン（64 文字 hex 等） |
| expires_at | datetime | - | 有効期限（未設定の場合は期限なし扱い） |
| used_at | datetime | - | 使用日時（未使用は null） |
| created_at | datetime | ✅ | 作成日時 |
| updated_at | datetime | ✅ | 更新日時 |

### 3.6 ヒアリング項目（HearingItem）
...
| 受講生 - ヒアリングトークン | 1:N | 同一受講生に複数のトークン発行履歴があり得る（再発行・使用済み） |
